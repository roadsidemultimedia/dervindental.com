	...
	for (m=0; m<_root.mainMenu_xml.item.length; m++) {
		...
		mainMenuItem.onPress = function() {
			if (this.linkType == "url") {
				...
				this.gotoAndPlay("out");
			} else if (this.linkType == "swf" || this.linkType == "gallery") {
...
function refreshMainMenu() {
	...
	// REFRESH SUBMENUS
	// if submenu items exist...
	if (_root.mainMenu_xml.item[_global.mainMenu.currentMain].item) { 
		// if submenu items are part of a gallery...
		if (_root.mainMenu_xml.item[_global.mainMenu.currentMain].type == "gallery") {
			// if a gallery menu is visible...
			if (_global.mainMenu.galleryMenuIsVisible == true) {
				// if gallery is not the one we want...
				if (_global.galleryMenu.id != _global.mainMenu.currentMain) {
					// refresh the gallery menu
					_global.mainMenu.initGalleryMenu(_global.galleryMenu);
				}
			// else if a gallery menu is not visible...
			} else if (_global.mainMenu.galleryMenuIsVisible != true) {
				// refresh the gallery menu
				_global.mainMenu.initGalleryMenu(_global.galleryMenu);
			}
		...
		// if submenu items are NOT part of a gallery...
		} else {
			if (_global.mainMenu.galleryMenuIsVisible == true) {
				// hide the gallery menu
				_global.mainMenu.hideGalleryMenu();
			}
			// if there is no currently visible submenu..
			if (_global.mainMenu.submenuIsVisible != true) {
				// reset selectedSubmenu, call showSubmenu function
				_global.mainMenu.selectedSubmenu = _global.mainMenu.currentMain;
				_global.mainMenu.showSubmenu();
			// if there is already a visible submenu
			} else { 
				// if the currently visible submenu is the one we want
				if (_global.mainMenu.selectedSubmenu == _global.mainMenu.currentMain) { 
	...
	// if NO submenu items exist...
		...
		// or if a gallery menu is visible...
		if (_global.mainMenu.galleryMenuIsVisible == true) {
			_global.mainMenu.hideGalleryMenu();
		}
	}
...
function showSubmenu() {
	...
	_global.mainMenu.refreshGalleryMenu();
}
